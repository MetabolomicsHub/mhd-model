...
class MhdGraph(MhdConfigModel):
    ...
    nodes: Annotated[list[GraphNode], Field()] = []
    relationships: Annotated[list[Relationship], Field()] = []

    @field_validator("nodes", mode="before")
    @classmethod
    def node_validator(cls, v, info: ValidationInfo) -> list[BaseMhdModel]:
    ...
    def get_node_class(
        ...
        if (
            ...
            return
        type_class_mapping = None
        if mhd_model_validation_context:
            type_class_mapping = mhd_model_validation_context.type_class_mapping or None
